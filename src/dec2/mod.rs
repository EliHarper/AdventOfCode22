...
impl RoshamboDecoder {
    ...
    fn _value_of_round(&self, combo: &str) -> i32 {
        ...
        let yours = self._get_shape_value(tup.0.to_string());
        let mine = self._get_shape_value(tup.1.to_string()); // Won't be used as shape value in challenge 2

        self._score_with_second_col_as_outcome(mine, yours)

        /* Challenge #1 */
        // self._score_with_second_column_as_shape(mine, yours)
    }

    fn _score_with_second_col_as_outcome(&self, mine: i32, yours: i32) -> i32 {
        // Draw
        if mine == 2 {
        ...
        let win = mine == 3;
        let shape_value = self._shape_value_from_ltw(win, yours).unwrap();
    ...
    fn _shape_value_from_ltw(&self, win: bool, yours: i32) -> Result<i32> {
        // r > s, s > p, p > r
        if win {
            return match yours {
    ...
    fn _get_shape_value(&self, choice: String) -> i32 {
...
pub fn _part_two() {
    ...
    for line in text.lines() {
        let line = line.trim();
        total_score += decoder._value_of_round(line);
    }
